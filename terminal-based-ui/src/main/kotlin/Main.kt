...
fun runGameLoop(engine: GameEngine) {
  ...
  while (true) {
    ...
    if (response.state.activePlayerId == humanId) {
      ...
      lastAction = action
      response = engine.handle(GameCommand.SubmitTurn(playerId = humanId, action = action))
    } else {
      println("Vez do oponente...")

      val botMove = botRuntime.decide(response.state, botId)
      lastAction = botMove.action
      response = engine.handle(GameCommand.SubmitTurn(playerId = botId, action = botMove.action))
    }
    ...
    response.investigationAnswer?.let { renderInvestigationResult(lastAction, it) }
...
fun parseHumanAction(input: String): TurnAction {
  val args = input
    ...
    .map { it.toIntOrNull() ?: error("Use apenas números") }
...
fun parseInvestigation(args: List<Int>): TurnAction {
  return when (args.firstOrNull()) {
    1 -> {
      require(args.size == 2)
      TurnAction.Investigate(InvestigationQuestion.DigitExists(Digit(args[1])))
    }

    2 -> {
      require(args.size == 3)
      TurnAction.Investigate(InvestigationQuestion.DigitAtPosition(PositionIndex(args[1]), Digit(args[2])))
    }
    ...
    3 -> {
      ...
      TurnAction.Investigate(InvestigationQuestion.PositionCharacteristic(PositionIndex(args[1]), characteristic))
    }

    4 -> {
      require(args.size == 3)
      TurnAction.Investigate(InvestigationQuestion.PositionComparison(PositionIndex(args[1]), PositionIndex(args[2])))
    }
...
fun parseCode(args: List<Int>): TurnAction {
  require(args.size == 4)
  return TurnAction.AttemptCode(Code.of(args[0], args[1], args[2], args[3]))
}
...
fun parseBlock(args: List<Int>): TurnAction {
  return when (args.firstOrNull()) {
    1 -> {
      ...
      TurnAction.Block(BlockAction.Investigation(target))
    }

    2 -> TurnAction.Block(BlockAction.AttemptClue)
    3 -> TurnAction.Block(BlockAction.EnergySurcharge)
...
fun renderState(state: GameState) {
  ...
  println()
  println("Vez atual: ${if (state.activePlayerId == human.id) "Você" else "Oponente"}")
  println()

  when (val block = state.pendingBlock) {
    null -> println("Efeito ativo: nenhum")

    is PendingBlockState.Investigation -> println("Efeito ativo: investigação bloqueada (${block.target})")

    PendingBlockState.AttemptClue -> println("Efeito ativo: dica reduzida")

    PendingBlockState.EnergySurcharge -> println("Efeito ativo: custo aumentado")
  }
...
fun renderInvestigationResult(
  ...
  val question = (action as? TurnAction.Investigate)?.question
  println()
  println(if (answer) "🔎 Resultado: SIM" else "🔎 Resultado: NÃO")
  question?.let { println("Pergunta: ${describeQuestion(it)}") }
}
